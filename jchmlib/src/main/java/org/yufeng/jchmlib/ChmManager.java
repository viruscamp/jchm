 ...
 *
 * @author yufeng
 *the main body to control the reading process of chm
 */

public class ChmManager {
    /**these four String is useful for us to get system information of a chm file
     */
    private static String CHMU_SPANINFO =
            "::DataSpace/Storage/MSCompressed/SpanInfo";
    private static String CHMU_RESET_TABLE =
            "::DataSpace/Storage/MSCompressed/Transform/{7FC28940-9D31-11D0-9B27-00A0C91E9C7C}/InstanceData/ResetTable";
    private static String CHMU_CONTENT =
            "::DataSpace/Storage/MSCompressed/Content";
    private static String CHMU_LZXC_CONTROLDATA =
     ...
     */
    public ArrayList enumerateFiles() {
        ArrayList tmp = enumerateAll();
    ...
    /**this will use the chm file name to construct a chmmanager object,
     *it initialize everything needed for chm reading
     */
    public ChmManager(String file) {
        ...
        spaninfo = getChmSeg(dataOffset.add(fe.offset).longValue(), fe.length.intValue()).getBytes(fe.length.intValue());
        fe = resolveObject(CHMU_RESET_TABLE);
        lrt = new ChmLzxcResetTable(getChmSeg(dataOffset.add(fe.offset).longValue(), fe.length.intValue()));
        ...
        clcd = new ChmLzxcControlData(getChmSeg(dataOffset.add(fe.offset).longValue(), fe.length.intValue()));
        windowSize = clcd.windowSize;
        lzxBlocksCache = new ArrayList();
        // firstCachedBlock=0;
        //int a=1;
    }
    ...
    public String retrieveFile(FileEntry fe) {
        ...
        try {
            for (int i = 0; i < b.length; i++) {
                tmp = tmp.concat(new String(b[i], encoding)); //should get return value
            }
    ...
    /**get any specified file,which could be a text file or a picture
     *the name should have no any different with
     *that listed in Pmgls, including path,
     */
    ...
    public byte[][] retrieveObject(FileEntry fe) {
        ...
        if (fe.compressType == CompressType.CHM_UNCOMPRESSED)    //uncompressed
            tmp[0] = (getChmSeg(dataOffset.add(fe.offset).longValue(), fe.length.intValue())).getBytes(fe.length.intValue());
        else if (fe.compressType == CompressType.CHM_COMPRESSED) //compressed
        {
            ...
            {
                start = bb.iniBlock;
                lzxBlock = new LzxBlock(start, windowSize, getLzxSeg(start), lrt.block_len, null);
            ...
            for (i = start; i <= bb.endBlock; ) {
                ...
                if (i == bb.startBlock) tmp[0] = lzxBlock.getContent(bb.startOffset);
                if (i > bb.startBlock && i < bb.endBlock) tmp[i - bb.startBlock] = lzxBlock.getContent();
                if (i == bb.endBlock) {
                    tmp[i - bb.startBlock] = lzxBlock.getContent(0, bb.endOffset);
                    break;
                }
                i++;
                if (i % (int) clcd.resetInterval == 0)
                    lzxBlock = new LzxBlock(i, windowSize, getLzxSeg(i), lrt.block_len, null);
                else lzxBlock = new LzxBlock(i, windowSize, getLzxSeg(i), lrt.block_len, lzxBlock);
                lzxBlocksCache.add(lzxBlock);
            }
    ...
    public void cleanBlocksCache(int blocks) {
        if (lzxBlocksCache.size() > blocks) {
            for (int i = 0; i < blocks / 5 + lzxBlocksCache.size() - blocks; i++) //减到最大允许值的4/5
    ...
    {
        if (block < 0 || block >= lrt.block_address.length) return null;
        int len;
        if (block < lrt.block_address.length - 1)//not last block
            len = lrt.block_address[block + 1].subtract(lrt.block_address[block]).intValue();
        else //lastblock
     ...
     *its offset and its length
     *and whether it's compressed
     */
    public FileEntry resolveObject(String filename) {
        ...
        if (cpi != null) {
            ...
            for (i = 0; i < cpi.indexEntries.size(); i++)
                if (filename.compareTo(((IndexEntry) cpi.indexEntries.get(i)).entryName) <= 0)
                    break;
    ...
    public ChmPmglSection resolvePmgl(int startpmgl) {
        if (startpmgl == -1) return null;
        byte[] b = new byte[(int) cip.dir_blocklen];
        // FileEntry ie=null;

        // fr.seek(dirOffset+startpmgl*cip.dir_blocklen);//the start point of first pmgl or pmgi
        // if(fr.read(b)==(int)cip.dir_blocklen) {
        ChmSeg cc = getChmSeg(dirOffset + startpmgl * cip.dir_blocklen, (int) cip.dir_blocklen);
    ...
    public ChmPmgiSection resolvePmgi() {
        ...
        if (cip.index_depth == 2) {
            ...
            // fr.seek(dirOffset+cip.index_root*cip.dir_blocklen);//the start point of first pmgl or pmgi
            // if(fr.read(b)==(int)cip.dir_blocklen) {
            ChmSeg cc = getChmSeg(dirOffset + cip.index_root * cip.dir_blocklen, (int) cip.dir_blocklen);
    ...
    /**  public boolean open(String file)  {
     ...
     cih=new ChmItsfHeader();
     if(!cih.setInfo(fr))
     return false;
     // dirlen=cih.getData_offset().toString();
     cip=new ChmItspHeader();
     if(!cip.setInfo(fr,cih.dir_offset.longValue()))
     return false;
     //dirlen=(new Long(cip.getDir_blocklen())).toString();
     cps=new ChmPmglSection();
     //System.out.print(cih.dir_offset.longValue()+cip.header_len);
     if(!cps.setInfo(fr, cih.dir_offset.longValue()+cip.header_len))
     return false;
     // dirlen=cps.free_space;
     cds=new ChmIndexSection();
     if(!cds.setSystemInfo(fr,cih.dir_offset.longValue()+cip.header_len+20, cih.data_offset.longValue()-cps.free_space))
     return false;
     lrt=new ChmLzxcResetTable();
     // System.out.println(cih.dir_offset.longValue()+cip.header_len+cds.chmuResetTable.offset.longValue());
     if(!lrt.setInfo(fr, cih.data_offset.longValue()+cds.chmuResetTable.offset.longValue(), cds.chmuResetTable.length.longValue(), 1))
     return false;// lzxcResetTable.setInfo(fr,start+data_offset);
     //comlen=cds.chmuResetTable.offset.toString();
     clcd=new ChmLzxcControlData();
     if(!clcd.setInfo(fr,cih.data_offset.add(cds.chmuLzxcControlData.offset),cds.chmuLzxcControlData.length))
     return false;
     // lzx=new Lzx(16, fr,cih, lrt, cds.chmuContent);
     return true;
     }
     */
}
...
class ChmItsfHeader {
    ...
    public ChmItsfHeader(ChmSeg cc) {
        if (cc.compareTo(signature) == 0) {
            ...
            lang_id = cc.getUInt();  /** 14 */
            dir_uuid = cc.getBytes(16);
            stream_uuid = cc.getBytes(16);
            unknown_offset = cc.getUlong();  /** 38 */
...
class ChmLzxcControlData {
    ...
    public ChmLzxcControlData(ChmSeg cc) {
        ...
        if (cc.compareTo(signature) == 0) {
            version = cc.getUInt();   //UInt32
            resetInterval = cc.getUInt();          /**  c        */
            windowSize = cc.getUInt();             /** 10        */
            windowsPerReset = cc.getUInt();        /** 14        */
...
class ChmCompressType {
    public static int Uncompressed=0;
    public static int compressed=1;
}
class SpanInfo {
    byte[] spanInfo=new byte[8];
    public boolean setInfo(RandomAccessChm fr,BigInteger start,BigInteger length) {
        try {
            fr.seek(start);
            fr.read(spanInfo);
        } catch(IOException e) {
    ...
    public boolean setInfo(RandomAccessChm fr,FileEntry ieobj) {
        if (ieobj.compressType==ChmCompressType.compressed)
            return false;
        else
            return setInfo(fr,ieobj.offset,ieobj.length);
    }
}
class ChmIndexSection {
    private static String  CHMU_SPANINFO =
            "::DataSpace/Storage/MSCompressed/SpanInfo";
    private static String CHMU_RESET_TABLE =
            "::DataSpace/Storage/MSCompressed/Transform/{7FC28940-9D31-11D0-9B27-00A0C91E9C7C}/InstanceData/ResetTable";
    ...
    private static String CHMU_LZXC_CONTROLDATA =
            "::DataSpace/Storage/MSCompressed/ControlData";
    FileEntry chmuSpaninfo=new FileEntry(CHMU_SPANINFO);
    FileEntry chmuResetTable=new FileEntry(CHMU_RESET_TABLE);
    FileEntry chmuContent=new FileEntry(CHMU_CONTENT);
    FileEntry chmuLzxcControlData=new FileEntry(CHMU_LZXC_CONTROLDATA);
    ArrayList fileEntries=new ArrayList();
    public boolean setSystemInfo(RandomAccessChm fr,long indexoffset, long len) {
        int j=0;
        try {
            fr.seek(indexoffset);
            long i=indexoffset;
            ...
            String name;
            while(i<len) {
                namelen=fr.readEncint().intValue();
                name=fr.readUtfString(namelen);
                if(name.compareTo(chmuSpaninfo.entryName)==0) {
                    chmuSpaninfo.compressType=fr.readEncint().intValue();
                    chmuSpaninfo.offset=fr.readEncint();
                    chmuSpaninfo.length=fr.readEncint();
                    j++;
                } else if (name.compareTo(chmuResetTable.entryName)==0) {
                    chmuResetTable.compressType=fr.readEncint().intValue();
                    chmuResetTable.offset=fr.readEncint();
                    chmuResetTable.length=fr.readEncint();
                    j++;
                    //long a=chmuResetTable.offset.longValue();
                    //long b=chmuResetTable.length.longValue();
                } else if(name.compareTo(chmuContent.entryName)==0) {
                    chmuContent.compressType=fr.readEncint().intValue();
                    chmuContent.offset=fr.readEncint();
                    chmuContent.length=fr.readEncint();
                    j++;
                } else if(name.compareTo(chmuLzxcControlData.entryName)==0) {
                    chmuLzxcControlData.compressType=fr.readEncint().intValue();
                    chmuLzxcControlData.offset=fr.readEncint();
                    chmuLzxcControlData.length=fr.readEncint();
                    j++;
                } else {
                    FileEntry tmp=new FileEntry(name);
                    tmp.compressType=fr.readEncint().intValue();
                    tmp.offset=fr.readEncint();
                    tmp.length=fr.readEncint();
                    fileEntries.add(tmp);
                }
                i=fr.getFilePointer();
            }
    ...
    public FileEntry getFileEntry(String filepath) {
        ...
        FileEntry tmp=null;
        for(i=0;i<fileEntries.size();i++) {
            tmp=(FileEntry)(fileEntries.get(i));
            if (tmp.entryName.compareTo(filepath)==0)
                break;
        }
        return tmp;
    }
