...
public class ChmReader {
    ...
    public static void extractOne(ChmManager cm, String htmlf, String savef) {
        byte[][] tmp = cm.retrieveObject(htmlf);//"/mini.GIF");///java/awt/CardLayout.html");///java/awt/AWTEvent.html");///javalogo52x88.gif");////test.html");//"/test.html");//"/内容.html");//content.html");//
        writeFile(tmp, savef);
                ...
                if ((c != null) && (c.length != 0)) {
                    ...
                    if (li != null) {
                        ...
                        for (int j = 0; j < li.length; j++) {
                            ...
                            if (cm == null)
                                System.out.println("can't open"); //hmm,it seemed that files that can't be opened is because they don't exist.
                            else {
