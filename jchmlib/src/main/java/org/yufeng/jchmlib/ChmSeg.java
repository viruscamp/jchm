 ...
 *
 * @author yufeng
 *it control any segments retrieved from chm
 *to easily handle
 */
public class ChmSeg {
    ...
    int remains;
    /**bit buffer.
     *is an int with bits less than 33 bits
     */
    int val;
    ...
    /**check the specifed position in bit buff is 1 or 0
     *bitstream will have 4 bytes
     */
    ...
    /**get byte but return int
     *it ensure that the leading three bytes don't change into fffff
     */
    public int getUByte() {
        //kicks++;
    ...
    public BigInteger getBigInteger(int i) {
        ...
        if (b.length - kicks < i) i = b.length - kicks;
        byte[] tmp = new byte[i];
        for (int j = i - 1; j >= 0; j--) {
            tmp[i - j - 1] = b[kicks + j];
        }
    ...
    /**compare to specified bytes,
     *if >orig, return integer t,e.g. 5 means that the 5th byte is >orig.
     **/
